#####################################################
#####################################################
########[05] Logistic Regression Lecture Code########
#####################################################
#####################################################
...
########################################################
GradSchools = read.table("./[05] Logistic Regression/[05] Graduate Schools.txt")
...
table(GradSchools$admit, GradSchools$rank)/nrow(GradSchools)*100

plot(GradSchools, col = GradSchools$admit + 2) #Basic graphical EDA.   # +2 is the colors
...
scatter.smooth(logit.overall$fit,
               ...
               main = "Residual Plot for\nLogistic Regression of Admission Data")   # residual plot is not that useful in general tho, so dont spend too much time on it
abline(h = 0, lty = 2)
...
# shows rank of college already in, not rank of grad school applying to
...
reject = filter(GradSchools, admit == 0)
admit = filter(GradSchools, admit == 1)

mean(predict(logit.overall, admit, type = 'response'))   # 0.38 admit
mean(predict(logit.overall, reject, type = 'response'))   # 0.28 reject

### but I still don't understand this
######

#Converting the fitted probabilities to binary:
admitted.predicted = round(logit.overall$fitted.values)    # with c = 50%
...
# overall dev test, prob less than 5%, so model is proper (?) not very different from before
