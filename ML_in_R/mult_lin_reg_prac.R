...
summary(states)    # no missing values, so well structured 
sapply(states, sd)
cor(states)
...
# fourth model shows problems with Hawaii and especially Alaska
...
# in graph, x axis "population | others" is residuals of predicted population based on other predictors (resid of lm(pop ~ .))
# y axis life expectancy based on others
...
# anova the reduced model and the full model 
# do the hypothesis test to confirm our selection makes sesne
...
anova(model.reduced, model.full) #The p-value is quite large; thus, the reduced
                                 #model is sufficient.
# did removing those make my predictor worse (p value is 78%, so removing those predictiors
# actually doesnt make the model much worse)

#Checking the model summary and assumptions of the reduced model.
summary(model.reduced)
plot(model.reduced)
influencePlot(model.reduced)
vif(model.reduced)   # everything is less than 2, so it is even better than before
...
# but we won't always get the same order for both BIC and AIC so be careful!
...
forwardAIC = step(model.empty, scope, direction = "forward", k = 2)   #murder rate is neg, so add it because it makes your AIC smaller by the most
# after adding murder, hs grad, frost, and population, -28.161 for <none> is best option, so we are done
backwardAIC = step(model.full, scope, direction = "backward", k = 2)
# remove one redictor at a time, similar to before but different
...
##################################
model.quadratic = lm(Test.Score ~ Hours.Studied + I(Hours.Studied^2), data = tests)  # make sure you put the I before the squared term 
...
# makes gender numeric by spreading the data to multiple columns 
# with the gender options and inputting one or zer for each observation

summary(model.factor) #Investigating the model and assessing some diagnostics.
plot(model.factor)
influencePlot(model.factor)

col.vec = c(rep("hotpink", 250), rep("blue", 250))
...
legend("topleft", c("Female Regression", "Male Regression"),
       lwd = 3, lty = 2, col = c("hotpink", "blue"))
